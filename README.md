...
#### :interrobang: 고민
```
삭제 기능이 있는 버튼들(선택삭제, 전체삭제 버튼)을 눌렀을 때
체크 표시가 활성화되어 있는 리스트만 삭제되는 기능을 만들고 싶었다.
```
#### :100: 해결 방법
```
체크 표시를 활성화하거나 비활성화하는 경우에
각각의 리스트의 css classList 목록에 on과 checked 클래스가 생기거나 사라지게 만들어두고,
삭제 버튼이 눌릴 당시에 각 리스트의 css classList 목록에 on과 checked 클래스가 존재한다면
그 리스트는 삭제하도록 코드를 구현했다.
```
<p align="center">
